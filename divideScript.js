...
const generateQuiz = () => {
    ...
    const falseAns1 = Math.floor(Math.random() * 13).toFixed(2)
    const falseAns2 = Math.floor(Math.random() * 13).toFixed(2)
    const falseAns3 = Math.floor(Math.random() * 13).toFixed(2)
    let allAnswers = []
    let switchAnswer = []
    answer = num1 / num2
    answer.toFixed(2)
